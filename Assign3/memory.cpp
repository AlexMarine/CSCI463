...
uint16_t memory::get16(uint32_t addr) const
{
  ...
  //combine the left and right for the full little endian order
  uint32_t littleEndian = (right << 8 | left);
...
uint32_t memory::get32(uint32_t addr) const
{
  ...
  //combine the left and right for little endian order
  uint32_t littleEndian = (right << 16 | left);
...
{
  //variable definition for the right side of the little endian order of a 16-bit valye
  uint8_t right = (val & 0x00ff);
  set8(addr, right);

  //variable definition for the left side of the little endian order of a 16-bit balue
...
{
  //variable definition for the right side of the little endian order of a 32-bit value
  uint16_t right = (val & 0x0000ffff);
  set16(addr, right);

  //variable definition for the left side of the little endian order of a 32-bit value
  uint16_t left = (val & 0xffff0000) >> 16;
  set16(addr + 2, left);
}

/**
 * @breif Dump the contens of the simulated memory in hex along with ASCII
 **********************************************************************************************/
void memory::dump() const
{
  ...
  {
    //print a '.' if the byte does not have a valid printable value
    uint8_t ch = get8(i);
...
bool memory::load_file(const std::string &fname)
{
  ...
  {
    //while the end of file has not been reached, continue
    while(!infile.eof())
    {
